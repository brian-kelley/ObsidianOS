...
int resetKeyboard()
{
  ...
  while(result == 0xFE)
  {
    keyboardCommand(0xFF);
    result = getKeyboardData();
    if(timeout-- == 0)
    {
      //too many retries, return failure
      return 1;
    }
  }
  //Command acknowledged, wait for self-test results
  //At this point, success or failure is known
  switch(result)
  {
    case 0xAA:
...
{
  //clear PS/2 data buffer, if it contains a byte
	while(readport(0x64 & 1))
		readport(0x60);
	int numRetries = 3;	//when reaches 0, time out and report error
	while(numRetries > 0)
	{
    keyboardCommand(0xF4);
		byte result = getKeyboardData();
		if(result == 0xFA)
			return 0;
		else if(result == 0xFE)
		{
...
void initKeyboard()
{
  ...
  //first, init IDT as all-empty (handler does nothing)
  for(int i = 0; i < 256; i++)
  {
    ...
    idt[i].offsetHigher = passAddr >> 16;
  }
	...
	idt[0x21].offsetHigher = keyboardAddress >> 16;
	if(resetKeyboard())
		puts("ERROR: Keyboard reset failed.");
	...
	idtPtr[0] = 2047 | ((idtAddress & 0xFFFF) << 16);
